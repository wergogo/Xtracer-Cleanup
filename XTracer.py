...
# Load configuration
_conf = XT_config.config('config/config.yml').data or {}
_base = _conf.get('baseConf', {})
hook_mode = _base.get('hook_mode', 'single')
chooseApkPath = _base.get('apk_dir', 'apks')
use_monkey = bool(_base.get('use_monkey', False))

storage_path = chooseApkPath + '/0000_storage.yml'
featurePath = os.path.join(chooseApkPath, 'feature')
...
def getApkPath(path):
    ...
    if not os.path.exists(path):
        printRed('[A] APK directory does not exist: ' + str(path))
        return apkPaths
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith('.apk'):
                apkPaths.append(os.path.join(root, file))
    return apkPaths
